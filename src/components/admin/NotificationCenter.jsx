...
export default function NotificationCenter({ unread = 0 }) {
  ...
  return (
    <div className="relative" ref={ref}>
      <button onClick={() => setOpen(!open)} className="relative p-2 rounded-lg hover:bg-white/10 transition-colors cursor-pointer bg-transparent border-none">
        <svg className="w-5 h-5 text-white/70" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
        </svg>
        {unreadCount > 0 && (
          <span className="absolute -top-0.5 -right-0.5 w-4.5 h-4.5 bg-red-500 text-white text-[9px] font-bold rounded-full flex items-center justify-center ring-2 ring-primary">
            {unreadCount}
          </span>
        )}
      </button>
                ...
                return (
                  <div key={n.id} className={`px-4 py-3 table-row ${isRead ? "opacity-60" : ""}`}>
                    <div className="flex items-start gap-3">
                      <div className={`w-2 h-2 rounded-full mt-1.5 shrink-0 ${typeColors[n.type] || "bg-slate-400"} ${isRead ? "" : "animate-pulse-dot"}`} />
                      <div className="flex-1 min-w-0">
                        <p className={`text-xs m-0 ${isRead ? "text-slate-500" : "text-slate-700 font-semibold"}`}>{n.title}</p>
                        <p className="text-[10px] text-slate-400 m-0 mt-0.5">{n.desc}</p>
                        <p className="text-[10px] text-slate-400 m-0 mt-0.5">{n.time}</p>
                      </div>
                    </div>
                  </div>
                );
