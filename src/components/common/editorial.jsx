...
export function Kicker({ children, className = "" }) {
  return <p className={`kicker m-0 ${className}`}>{children}</p>;
}
...
export function DisplayTitle({ as = 2, children, className = "" }) {
  ...
  return <Tag className={`${size} text-ink m-0 ${className}`}>{children}</Tag>;
}

export function Lead({ children, className = "" }) {
  return <p className={`lead m-0 ${className}`}>{children}</p>;
}

export function ArrowLink({ children, to, onClick, className = "", mute = false }) {
  const base = `group inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[0.18em] no-underline transition-colors ${
    mute ? "text-muted hover:text-ink" : "text-brand-deep hover:text-brand"
  } ${className}`;
  const arrow = (
...
export function SectionHeading({
  ...
  return (
    <div className={`flex flex-col gap-5 max-w-3xl ${alignCls} ${className}`}>
...
export function Wordmark({ children, className = "" }) {
  return (
    <span
      aria-hidden="true"
      className={`select-none pointer-events-none font-black tracking-tighter leading-none ${
    ...
    <div className={`grid gap-px bg-line border border-line overflow-hidden rounded-2xl ${cols} ${className}`}>
      {items.map((item, i) => (
        <div key={item.label ?? i} className="bg-paper p-6 md:p-8">
          <p className="m-0 text-[11px] font-bold uppercase tracking-[0.22em] text-muted">
            {item.label}
          </p>
          <p className="mt-4 m-0 text-4xl md:text-5xl font-extrabold tracking-tighter text-ink">
            {item.value}
          </p>
          {item.hint ? (
            <p className="mt-2 m-0 text-sm text-faint">{item.hint}</p>
          ) : null}
        </div>
      ))}
    </div>
