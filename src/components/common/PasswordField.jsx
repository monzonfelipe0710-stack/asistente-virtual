    ...
    <label className={`block ${animate ? "animate-field-in" : ""}`}>
      ...
      <div className="relative">
        <input
          ...
          required
          className="w-full px-4 py-3 pr-12 rounded-xl border border-line bg-paper text-sm text-ink placeholder:text-faint transition-colors duration-200 focus:border-brand focus:ring-2 focus:ring-brand/15 focus:outline-none"
        />
        <button
          type="button"
          ...
          className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center justify-center w-9 h-9 rounded-lg text-faint hover:text-brand-deep hover:bg-mist transition-colors cursor-pointer"
        >
          {visible ? (
